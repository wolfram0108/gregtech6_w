...
**Вы смотрите ветку `1.20.1`** — это бэкпорт: тот же мод и тот же генератор, собранные против
более старого движка. Ветки никогда не сливаются; исправление, которое касается обеих, вносится в обе.
...
Нужны **JDK 25** для запуска сборки (сам мод компилируется под **Java 17**, её Gradle достаёт сам),
около **8 ГБ свободной оперативной памяти** и **10 ГБ на диске**. Первая сборка
...
**В этом репозитории тестов нет, и у этой ветки их нет и рядом.** Проверка — это оснастка, а не
продукт: внутриигровые стенды живут в рабочей среде разработчика (`../stands-1.20.1/`), а
автоматические продуктовые тесты пока существуют только для ветки `main` — здесь `./gradlew test`
не находит, что запускать. Проверенное на этой ветке проверено стендами и сверкой с оригиналом,
