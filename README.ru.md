...
Minecraft 1.7.10 — на Minecraft 1.20.1 / NeoForge.**
...
- [Какая сборка нужна именно вам](#какая-сборка-нужна-именно-вам)
...
## Какая сборка нужна именно вам

Одна кодовая база, две ветки — выбирайте по версии игры, в которую играете:
...
**Вы смотрите ветку `1.20.1`** — тот же мод и тот же генератор, собранные против
более старого движка.
...
GregTech 6 написан под Minecraft 1.7.10 (2014 год). Между той версией и 1.20.1 практически каждая
...
Нужен установленный **JDK 17**, около **8 ГБ свободной оперативной памяти** и **10 ГБ на диске**. Первая сборка
...
Сборка скачивает только платформу: Gradle, NeoForge, Minecraft и их библиотеки. Чужие моды и JDK она
не скачивает никогда. Интеграции с другими модами компилируются против объявлений в
`src/compat-mirror/provided`, которые в jar не попадают.

### Сборка без сети

Если рядом с репозиторием лежит каталог `platform` (`../platform`), `gradlew` переводит сборку в
локальный режим: ничего не скачивается, все входы берутся из этого каталога. Без него сборка работает
как описано выше. В репозиторий каталог не входит — в нём лежат файлы Minecraft, распространять которые
нельзя, — поэтому он собирается самостоятельно из однажды скачанных файлов:

| Путь внутри `../platform` | Содержимое |
|---|---|
| `gradle/` | распакованный дистрибутив Gradle той версии, что названа в `gradle/wrapper/gradle-wrapper.properties` |
| `maven/` | хранилище в раскладке Maven со всеми плагинами и зависимостями сборки, включая файлы POM и module |
| `minecraft/resolved/launcher_manifest.json` | манифест лаунчера, в котором запись версии и каждый адрес внутри манифеста этой версии — URI вида `file:` |
| `minecraft/assets/objects/` | объекты ассетов игры, разложенные по хешу, как их хранит лаунчер |
| `jdk/17/` | JDK 17; используется, когда не задана `JAVA_HOME_17_X64` |
| `jdk/21/` | JDK 21 — на нём сборочный плагин запускает конвейер Minecraft; используется, когда не задана `JAVA_HOME_21_X64` |
| `jdk/25/` | JDK 25; используется, когда не заданы `JAVA_HOME` или `JAVA_HOME_25_X64` |
...
Есть один опциональный флаг сборки — `-Pgt6probes`: он подключает внутриигровые проверочные стенды.
Они живут **вне этого репозитория**, в рабочей среде разработчика (`../stands-1.20.1`); репозиторий не
содержит ни одного, поэтому попасть в jar игрока они не могут. При наличии того каталога и флага
`runClient`/`runServer` умеют прогонять автоматические проверки прямо в игре.
...
| **Applied Energistics 2** | поддержан — GT6 остаётся промышленным слоем, AE2 держит сеть, хранение, автокрафт и пространственное хранилище; машины AE2, повторяющие гректеховские, лишены своих рецептов, энергия ходит в обе стороны, гречий ключ крутит и разбирает блоки AE2 |
| **JourneyMap** и ванильная карта | поддержаны — блоки, руды, машины и жидкости GT6 отрисовываются на обеих; проверено живым модом `1.20.1-6.0.2+forge` |
