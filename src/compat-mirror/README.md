...
не должен существовать здесь ни одним классом: `ModuleClassLoader.java:76-78` (fml) держит
`packageLookup.put(pk, moduleInfo)` — один пакет = один модуль, второй молча затирается, порядок обхода
`HashMap` недетерминирован. Первый случай — `appeng.*`: зеркала удалены при подключении AE2 26.1
(этап Э0 миссии «слой совместимости GT6 ⇄ AE2», `build.gradle` → `maven.modrinth:ae2`).
...
- **Код ядра НЕ меняется, ПОКА мод не подключён** — `implements <чужой интерфейс>` остаётся дословным (1:1).
  ...
  (пример: `IMovableTile` у AE2 26.1 не существует — снят с `TileEntityBase01Root`, методы Грега целы).
...
**Обновление (2026-08-12, Э0 миссии AE2):** пять зеркал `appeng.*` (`api/AEApi`, `api/movable/IMovableTile`,
`api/recipes/IGrinderRecipeHandler`, `api/registries/IRegistries`, `tile/powersink/IC2`) **удалены** — AE2 26.1
подключён настоящим jar. Их потребители разведены: `IMovableTile` снят с `TileEntityBase01Root` (методы
`prepareToMove`/`doneMoving` Грега остались обычными), `AEApi`+grinder-реестр сняты вместе с носителем
(кварцевой мельницы в AE2 26.1 нет), `powersink.IC2` — вместе с флагом `EnergyCompat.AE_ENERGY`.
...
|---|---|---|
| ~~`appeng.api.movable.IMovableTile`~~ | ~~`TileEntityBase01Root`~~ | ⛔ СНЯТО (Э0 миссии AE2): подключён настоящий AE2 26.1 |
