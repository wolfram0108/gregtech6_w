...
⛔ **ЗЕРКАЛО В `java/` СНИМАЕТСЯ, КАК ТОЛЬКО МОД ПОДКЛЮЧЁН ПО-НАСТОЯЩЕМУ** (подключение — через `provided/`, см. последний раздел). Пакет, которым владеет реальный jar,
...
## `provided/` — заготовки модов, которые есть в игре настоящими jar

Сборка не скачивает чужие моды. JEI, Jade, AE2 и More Red присутствуют в игре собственными jar, а код мода
компилируется против заготовок в `provided/`: по одной на чужой тип, только сигнатуры, которые мод
вызывает или реализует. Сами jar для dev-запусков лежат рядом с репозиторием, в `../stands-1.20.1/_mods`.

| Правило | Чем обеспечено |
|---|---|
| заготовка собирается отдельным набором исходников и не попадает ни в вывод мода, ни в jar — в игре этими пакетами владеет настоящий мод | `build.gradle` (`sourceSets.provided`), локальная проверка jar до публикации |
| сигнатуры совпадают с настоящими до байта: jar, собранный против заготовок, равен jar, собранному против настоящих модов | локальный судья до публикации: та же сборка против настоящих jar из `../stands-1.20.1/_mods`, архивы сверяются побайтно |
| стенды (`-Pgt6probes`) читают внутренности модов, поэтому компилируются против настоящих jar из `../stands-1.20.1/_mods` | `build.gradle` |
| сборочные скрипты не объявляют сетевых источников | локальная проверка до публикации |

Новый вызов чужого API — объявление в заготовку того же типа с сигнатурой из jar этого мода
(`javap -p`), затем сравнение jar. Удержание аннотации, дескрипторы и мосты берутся как у
настоящего класса: любое расхождение меняет байткод мода.
