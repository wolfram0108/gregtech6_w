...
/** Compile-only mirror of the IC2 API: only the constructor CompatIC2 actually calls;
 *  the rest of the real class is unused and left out. */
public class RecipeInputOreDict implements IRecipeInput {
	public String input; // actually read by GregTech6's Loader_Recipes_Replace as an ore-dictionary string
	public RecipeInputOreDict(String aOreDict, int aAmount) {input = aOreDict;}
	@Override public boolean matches(net.minecraft.world.item.ItemStack aStack) {return false;} // compile-only mirror; the real mod never loads
}
