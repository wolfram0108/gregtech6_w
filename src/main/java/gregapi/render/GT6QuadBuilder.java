...
public final class GT6QuadBuilder {
	 ...
	 * про эту грань вопрос соседу (BP-BUG-019).
	 *
	 ...
	 * границы — ваниль 1.20.1 использует, назначая граням модели {@code cullface}. Так что это не новое
	 * правило, а то же самое, перенесённое на per-face уровень: движок сам сверит формы
	 * ({@code Block.shouldRenderFace} → {@code skipRendering} → сравнение occlusion-форм), и внутренняя
	 * грань (верх слэба) останется видимой, потому что её плоскость границы не касается.
	 */
