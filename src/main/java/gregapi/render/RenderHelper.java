...
public class RenderHelper {
	...
	public static void renderItemIntoGUI(Font aFontRenderer, TextureManager aTextureManager, ItemStack aStack, int aX, int aY, boolean aEffect) {
		...
		if (aStack.getItemSpriteNumber() == 0 && RenderBlocks.renderItemIn3d(Block.getBlockFromItem(aStack.getItem()).getRenderType())) {
			aTextureManager.bindTexture(TextureAtlas.locationBlocksTexture);
			...
			Identifier tResource = aTextureManager.getResourceLocation(aStack.getItemSpriteNumber());
			...
			for (int i = 0; i < tMaxRenderPasses; i++) {
				...
				if (tIcon == null) tIcon = ((TextureAtlas)Minecraft.getMinecraft().getTextureManager().getTexture(tResource)).getAtlasSprite("missingno");
