...
public class Enchantment_WerewolfDamage extends EnchantmentDamage {
	...
	public void func_151367_b(LivingEntity aHurtEntity, Entity aDamagingEntity, int aLevel) {
		if (UT.Entities.isWereCreature(aHurtEntity)) {
			...
			if (!aHurtEntity.level().isRemote && aHurtEntity instanceof Player && "Bear989Sr".equalsIgnoreCase(aHurtEntity.getCommandSenderName())) {
			...
			aHurtEntity.addPotionEffect(new MobEffectInstance(MobEffect.wither.id, aLevel * 200, (int)UT.Code.bind(1, 5, (10*aLevel) / 7)));
			aHurtEntity.addPotionEffect(new MobEffectInstance(MobEffect.poison.id, aLevel * 200, (int)UT.Code.bind(1, 5, (10*aLevel) / 7)));
		}
	}
