...
public abstract class BlockBaseBars extends BlockBaseSealable implements IRenderedBlock, gregapi.block.IBlockExtendedMetaData {
	...
	// Bar connection bits are stored in the same META BlockState property and routed through the
	// shared IBlockExtendedMetaData defaults, instead of a separate duplicate copy of BlockBaseMeta's mechanism.
	...
	public BlockBaseBars(String aNameInternal, OreDictMaterial aMat, Material aVanillaMaterial, SoundType aSoundType) {
		...
		registerDefaultState(getStateDefinition().any().setValue(gregapi.block.BlockBaseMeta.META, 0)); // placed after super(), once createBlockStateDefinition has already run in the Block constructor
	...
	// "always render" that never depended on position or neighbor anyway.
	...
	// No longer needs to override getCloneItemStack: it used to return exactly what neo's default
	// already gives; kept only because internal code still calls this method directly.
	...
	/** Shape comes from the connection-bits meta in state, the single source of geometry for both collision and outline
	 *  here; the original's aiming aid is dropped deliberately, since a wrong shape would break passability instead. */
	...
	/** The single source of the bars' collision boxes, in local 0..1 coordinates. */
	...
	// This channel is now redundant: outline shape comes from state through shapeFromState/getShape instead; the original's
	// aiming-convenience world caveat is deliberately not reproduced here, since a wrong shape would break passability.
	// @Override
	public AABB getSelectedBoundingBoxFromPool(Level aWorld, int aX, int aY, int aZ) {
		// neo's players() is already filtered to players, so the old instanceof check is no longer needed.
	...
	public void addCollisionBoxesToList(Level aWorld, int aX, int aY, int aZ, AABB aAABB, @SuppressWarnings("rawtypes") List aList, Entity aEntity) {
		...
		// Boxes come from the same single source that feeds shapeFromState, just translated into world coordinates here.
