...
public class MultiTileEntityBlock extends Block implements IBlock, IItemGT, IBlockDebugable, IBlockErrorable, IBlockOnWalkOver, IBlockSyncDataAndCoversAndIDs, IRenderedBlock, EntityBlock, IBlockToolable, IBlockRetrievable, IBlockMaterial {
	...
	private static BlockEntity reuseOrCreateClientTE(Level aWorld, int aX, int aY, int aZ, short aID1, short aID2) {
	...
	@Override public final void receiveData         (BlockGetter aWorld, int aX, int aY, int aZ              , INetworkHandler aNetworkHandler, short aID1, short aID2)                                                                {if (!(aWorld instanceof Level)) return; BlockEntity aTileEntity = reuseOrCreateClientTE((Level)aWorld, aX, aY, aZ, aID1, aID2); if (aTileEntity == null) return; if (aTileEntity instanceof ITileEntitySynchronising) ((ITileEntitySynchronising)aTileEntity).processPacket(aNetworkHandler); WD.update(aWorld, aX, aY, aZ);}
	...
	public final void onNeighborBlockChange(Level aWorld, int aX, int aY, int aZ, Block aBlock) {BlockEntity aTileEntity = WD.te(aWorld, aX, aY, aZ, T); if (!LOCK) {LOCK = T; if (aTileEntity instanceof ITileEntity) ((ITileEntity)aTileEntity).onAdjacentBlockChange(aX, aY, aZ); LOCK = F;} if (aTileEntity instanceof IMTE_OnNeighborBlockChange) ((IMTE_OnNeighborBlockChange)aTileEntity).onNeighborBlockChange(aWorld, aBlock); if (aTileEntity == null) {if (aWorld.isClientSide()) OUT.println("[GT6-ORPHAN-SKIP] клиент: BE ещё не доехал @" + aX + "," + aY + "," + aZ + " — блок СОХРАНЁН (до фикса удалялся)"); else WD.set(aWorld, aX, aY, aZ, NB, 0, 3);}}
