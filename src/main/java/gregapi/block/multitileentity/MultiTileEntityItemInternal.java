...
public class MultiTileEntityItemInternal extends BlockItem implements squeek.applecore.api.food.IEdible, IItemReactorRod, IItemUpdatable, IItemColorableRGB, IOreDictItemDataOverrideItem, IItemGT, IItemNoGTOverride, IFluidContainerItem, ISpecialElectricItem, IElectricItemManager, IItemEnergy, IItemElectric, IItemRottable, IFlowerPlaceable {
	...
	// F1 contract (1.7.10 itemDamage==meta): the verbatim GT6 code (getUnlocalizedName etc.) calls getDamage for the SUBTYPE
	// (machine ID); the neo default (IItemExtension) reads the DAMAGE component = 0 for meta items → all names/data get ID 0.
	// Restoring the root contract: a non-damageable stack → subtype meta (the F1 center ST.meta_). Same as on other roots of the hierarchy.
			...
			&& WD.set(aWorld, aX, aY, aZ, aMTEContainer.mBlock, 15-aMTEContainer.mBlockMetaData, 2)) {
				...
				try {
					if (!aWorld.isClientSide()) {
						...
						// World.java:695-698) -> Level.updateNeighborsAt(BlockPos,Block,Orientation) [Level.java:338], the same
						...
						// was World.func_147453_f(x,y,z,Block) -> Level.updateNeighborsAt(BlockPos,Block,Orientation) [Level.java:338]
	...
	public InteractionResult use(Level aWorld, Player aPlayer, InteractionHand aHand) {
		...
		if (rStack != aStack) {aPlayer.setItemInHand(aHand, gregapi.util.ST.nn(rStack)); return InteractionResult.SUCCESS;} // F15: the 1.7.10 contract can return null — into the engine's hand only through the center
	...
	public int getUseDuration(ItemStack aStack, LivingEntity aEntity) {
		return getMaxItemUseDuration(aStack); // was Item.getMaxItemUseDuration(ItemStack) (1.7.10) -> neo Item.getUseDuration(ItemStack,LivingEntity) (Item.java:328)
	}
