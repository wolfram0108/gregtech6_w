...
public class Behavior_Place_Path extends AbstractBehaviorDefault {
	...
	public boolean onItemUse(MultiItem aItem, ItemStack aStack, Player aPlayer, Level aWorld, int aX, int aY, int aZ, byte aSide, float aHitX, float aHitY, float aHitZ) {
		...
		if (aBlock == Blocks.GRASS_BLOCK || gregapi.data.CS.Flattened.headOf(aBlock) == Blocks.DIRT || aBlock == Blocks.MYCELIUM || aBlock == BlocksGT.Grass || IL.BoP_Grass_Long.equal(aBlock) || IL.BoP_Grass_Origin.equal(aBlock) || IL.AETHER_Grass_Enchanted_Vanilla.equal(aBlock)) {
