...
public class PrefixItemProjectile extends PrefixItem implements IItemProjectile {
	...
	public ItemStack onDispense(BlockSource aSource, ItemStack aStack) {
		...
		Direction tFacing = aSource.getBlockState().getValue(DispenserBlock.FACING); // was func_149937_b(getBlockMetadata()) -> BlockSource.state()/DispenserBlock.FACING, same technique as ItemArmorBase.java:187
		...
		DefaultDispenseItemBehavior.spawnItem(aSource.getLevel(), itemstack1, 6, enumfacing, iposition); // was doDispense -> spawnItem (DefaultDispenseItemBehavior.java:30)
		return aStack;
	}
