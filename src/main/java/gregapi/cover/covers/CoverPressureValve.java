...
public class CoverPressureValve extends AbstractCoverAttachment {
		...
		if (aIsServerSide && !aData.mStopped && aTimer > 2 && aData.mTileEntity instanceof MultiTileEntityPipeFluid) {
			...
			if (tTank.isFull()) {
				...
				if (tDelegator.mTileEntity != null) {
					...
					try {for (Entity tEntity : (ArrayList<Entity>)aData.mTileEntity.getWorld().getEntitiesOfClass(Entity.class, aData.box(-2, -2, -2, +3, +3, +3))) UT.Entities.applyTemperatureDamage(tEntity, FL.temperature(tTank.getFluid()), 2.0F, 10.0F);} catch(Throwable e) {e.printStackTrace(ERR);}
	...
	public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
		...
		aList.add(LH.Chat.CYAN + LH.tt("Releases Fluids when Pipe is full."));
		aList.add(LH.Chat.ORANGE + LH.tt("Liquids require Tank in front!"));
		aList.add(LH.Chat.ORANGE + LH.tt("Gases require Air or Tank in front!"));
