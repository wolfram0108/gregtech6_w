			...
			if (aChatReturn != null) {
				if (aData.mNBTs[aCoverSide] == null) {
					...
					if (ST.invalid(tStack)) {
						...
						aChatReturn.add(LH.tt("Exports: ") + LH.Chat.CYAN + ST.regName(tStack) + LH.Chat.GRAY + " ; " + LH.Chat.CYAN + ST.meta_(tStack) + LH.tt(" (Priority: ") + aData.mValues[aCoverSide] + ")");
					}
				}
		...
		if (aPlayer instanceof Player && aData.mTileEntity.isServerSide()) {
			if (aData.mNBTs[aCoverSide] == null || !aData.mNBTs[aCoverSide].contains("gt.filter.item")) {
				...
				if (ST.valid(tStack)) {
					...
					UT.Entities.sendchat(aPlayer, LH.tt("Exports: ") + LH.Chat.CYAN + ST.regName(tStack) + LH.Chat.GRAY + " ; " + LH.Chat.CYAN + ST.meta_(tStack));
				}
			}
		}
		return T;
