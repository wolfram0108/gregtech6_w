...
public class CoverTextureCanvas extends AbstractCoverDefault {
	...
	public void onCoverPlaced(byte aSide, CoverData aData, Entity aPlayer, ItemStack aCover) {
		...
		if (aCover != null && ItemNBT.has(aCover)) aData.visual(aSide, (short)((net.minecraft.core.registries.BuiltInRegistries.BLOCK.getId(ST.getBlock(ItemNBT.get(aCover), NBT_CANVAS_BLOCK)) << 4) | (ItemNBT.get(aCover).getIntOr(NBT_CANVAS_META, 0) & 15)));
	...
	public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
		if (aStack != null && ItemNBT.has(aStack) && ItemNBT.get(aStack).contains(NBT_CANVAS_BLOCK)) {
			...
			aList.add(LH.Chat.CYAN + LH.tt("Block Image: ") + ST.names(ST.make(ST.getBlock(ItemNBT.get(aStack), NBT_CANVAS_BLOCK), 1, ItemNBT.get(aStack).getIntOr(NBT_CANVAS_META, 0) & 15)));
		}
