			...
			if (aChatReturn != null) {
				if (aData.mNBTs[aCoverSide] == null) {
					...
					if (tFluid == null) {
						...
						aChatReturn.add(LH.tt("Exports: ") + LH.Chat.CYAN + FL.regName(tFluid.getFluid()) + LH.tt(" (Priority: ") + aData.mValues[aCoverSide] + ")");
					}
				}
			}
			return 1;
			...
			if (aData.mNBTs[aCoverSide] == null || !aData.mNBTs[aCoverSide].contains("gt.filter.fluid")) {
				...
				if (ST.valid(tStack)) {
					...
					if (FL.valid(tFluid)) {
						...
						UT.Entities.sendchat(aPlayer, LH.tt("Exports: ") + LH.Chat.CYAN + FL.regName(tFluid.getFluid()));
					}
				}
			}
		}
