...
public class RM {
	...
	public static boolean mortarize(long aPower, ItemStack aInput, ItemStack aOutput1, ItemStack aOutput2) {
		...
		// Э0 (слой AE2): строка ae_grinder(5*aPower, …) снята — у AE2 под 1.20.1 нет кварцевой мельницы вовсе
		// (grep grindstone/Grindstone/GrinderRecipe по её дереву даёт 0 файлов), реестр рецептов, который она
		// наполняла, исчез вместе с ней — как и сам AEApi.
		te_pulverizer(UT.Code.bindInt(1000*aPower), aInput, aOutput1, aOutput2);
		ic2_macerator(aInput, aOutput1);
		return T;
	}
	
	// Э0 (слой AE2): три метода ae_grinder(…) сняты вместе со своим носителем. Они писали в реестр
	// AEApi.instance().registries().grinder() — кварцевую мельницу AE2 rv2; у AE2 под 1.20.1 машины нет,
	...
	public static void te_furnace(int energy, ItemStack input, ItemStack output) {
		...
		ST.writeToNBT(input, toSend.getCompound("input"));
		ST.writeToNBT(output, toSend.getCompound("output"));
		InterModComms.sendTo("ThermalExpansion", "FurnaceRecipe", () -> toSend);
	}
	...
	public static void te_pulverizer(int energy, ItemStack input, ItemStack primaryOutput, ItemStack secondaryOutput, int secondaryChance) {
		...
		ST.writeToNBT(input, toSend.getCompound("input"));
		ST.writeToNBT(primaryOutput, toSend.getCompound("primaryOutput"));
		if (secondaryOutput != null) ST.writeToNBT(secondaryOutput, toSend.getCompound("secondaryOutput"));
		toSend.putInt("secondaryChance", secondaryChance);
		InterModComms.sendTo("ThermalExpansion", "PulverizerRecipe", () -> toSend);
	}
	...
	public static void te_sawmill(int energy, ItemStack input, ItemStack primaryOutput, ItemStack secondaryOutput, int secondaryChance) {
		...
		if (secondaryOutput != null) ST.writeToNBT(secondaryOutput, toSend.getCompound("secondaryOutput"));
		toSend.putInt("secondaryChance", secondaryChance);
		InterModComms.sendTo("ThermalExpansion", "SawmillRecipe", () -> toSend);
	}
	...
	public static void te_smelter(int energy, ItemStack primaryInput, ItemStack secondaryInput, ItemStack primaryOutput, ItemStack secondaryOutput, int secondaryChance) {
		...
		toSend.put("primaryOutput", UT.NBT.make());
		toSend.put("secondaryOutput", UT.NBT.make());
		ST.writeToNBT(primaryInput, toSend.getCompound("primaryInput"));
		ST.writeToNBT(secondaryInput, toSend.getCompound("secondaryInput"));
		ST.writeToNBT(primaryOutput, toSend.getCompound("primaryOutput"));
		if (secondaryOutput != null) ST.writeToNBT(secondaryOutput, toSend.getCompound("secondaryOutput"));
		toSend.putInt("secondaryChance", secondaryChance);
		InterModComms.sendTo("ThermalExpansion", "SmelterRecipe", () -> toSend);
	}
	public static void te_smelter_ore(OreDictMaterial aMaterial) {
		CompoundTag toSend = UT.NBT.make();
		toSend.putString("oreType", aMaterial.toString());
		InterModComms.sendTo("ThermalExpansion", "SmelterBlastOreType", () -> toSend);
	}
	public static void te_crucible(int energy, ItemStack input, FluidStack output) {
		...
		toSend.put("output", UT.NBT.make());
		ST.writeToNBT(input, toSend.getCompound("input"));
		FL.writeToNBT(output, toSend.getCompound("output")); // output = FluidStack (не ItemStack) -> FL.writeToNBT (FL.java:1148)
	...
	public static void te_fill(int energy, ItemStack input, ItemStack output, FluidStack fluid, boolean reversible) {
		...
		FL.writeToNBT(fluid, toSend.getCompound("fluid"));
