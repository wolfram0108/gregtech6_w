...
public class LH {
	...
	// Required harvest TIER as a separate string — needed by tooltip-mod displays (Jade): the original
	// format prints the level only starting from 2 and always together with the tool name, while the
	// display needs just the tier. The key lives here, like every other string of the mod: scattering
	// string literals across plugins is not our way.
...
//      if (aEnergyType == TD.Energy.KU) return Chat.ORANGE + LH.get(LH.ACCEPTS_REDSTONE_FLUX_LOSS)+" 50%";
	...
	public static class Chat {
		...
		static {
			for (ChatFormatting tEnum1 : ChatFormatting.values()) {
				// Literally just formatting without Text in it.
				BASICALLY_EMPTY_STRINGS.add(    tEnum1.toString()    );
				BASICALLY_EMPTY_STRINGS.add(    tEnum1.toString()+" ");
				BASICALLY_EMPTY_STRINGS.add(" "+tEnum1.toString()    );
				for (ChatFormatting tEnum2 : ChatFormatting.values()) {
					// Literally just formatting without Text in it.
		...
		public static final String
		   BLACK          = ChatFormatting.BLACK.toString()
		,  DBLUE          = ChatFormatting.DARK_BLUE.toString()
		,  DGREEN         = ChatFormatting.DARK_GREEN.toString()
		,  DCYAN          = ChatFormatting.DARK_AQUA.toString()
		,  DRED           = ChatFormatting.DARK_RED.toString()
		,  PURPLE         = ChatFormatting.DARK_PURPLE.toString()
		,  ORANGE         = ChatFormatting.GOLD.toString()
		,  GOLD           = ChatFormatting.GOLD.toString()
		,  GRAY           = ChatFormatting.GRAY.toString()
		,  DGRAY          = ChatFormatting.DARK_GRAY.toString()
		,  BLUE           = ChatFormatting.BLUE.toString()
		,  GREEN          = ChatFormatting.GREEN.toString()
		,  CYAN           = ChatFormatting.AQUA.toString()
		,  RED            = ChatFormatting.RED.toString()
		,  PINK           = ChatFormatting.LIGHT_PURPLE.toString()
		,  YELLOW         = ChatFormatting.YELLOW.toString()
		,  WHITE          = ChatFormatting.WHITE.toString()
		,  OBFUSCATED     = ChatFormatting.OBFUSCATED.toString()
		,  BOLD           = ChatFormatting.BOLD.toString()
		,  STRIKETHROUGH  = ChatFormatting.STRIKETHROUGH.toString()
		,  UNDERLINE      = ChatFormatting.UNDERLINE.toString()
		,  ITALIC         = ChatFormatting.ITALIC.toString()
		,  RESET          = ChatFormatting.RESET.toString()
