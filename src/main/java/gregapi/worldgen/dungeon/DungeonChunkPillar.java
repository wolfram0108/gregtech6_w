	...
	public boolean generate(DungeonData aData) {
		...
		for (int tY = -3; aData.mY+tY >= 2 && temp; tY--) {
			...
			for (int tX = 6; tX <= 9 && !temp; tX++) for (int tZ = 6; tZ <= 9 && !temp; tZ++) {
				...
				if (tBlock instanceof FallingBlock || !WD.opaque(tBlock) || tBlock == BlocksGT.Sands || tBlock == BlocksGT.Diggables || tBlock == net.minecraft.world.level.block.Blocks.MUD) temp = T;
			}
			if (temp) {
