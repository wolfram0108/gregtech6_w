...
public class Config implements Runnable {
	...
	public final ModConfigSpec mConfig;
	...
	public Config(File aDirPath, String aFileName) {
		...
		mConfig = new ModConfigSpec(tPathUsed);
		mConfig.load();
		mConfig.save();
	...
	@Deprecated public Config(File aConfig) {this(new ModConfigSpec(aConfig));}
	@Deprecated public Config(ModConfigSpec aConfig) {mConfig = aConfig; mConfig.load(); mConfig.save(); Abstract_Mod.sConfigs.add(this);}
	...
	public static ModConfigSpec sConfigFileIDs;
	private static boolean sIDConfigNeedsSaving = F;
	
	public static int addIDConfig(Object aCategory, String aName, int aDefault) {
		if (UT.Code.stringInvalid(aName)) return aDefault;
		ConfigValue tProperty = sConfigFileIDs.get(aCategory.toString().replaceAll("\\|", "."), aName.replaceAll("\\|", "."), aDefault);
