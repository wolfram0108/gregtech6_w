...
public abstract class TileEntityBase08Barrel extends TileEntityBase07Paintable implements IMTE_AddToolTips, IMTE_GetMaxStackSize, ITileEntityFunnelAccessible, ITileEntityTapAccessible, ITileEntityProgress, ITileEntityConnectedTank, IFluidHandler, IFluidContainerItem, IItemRottable {
	...
	public void readFromNBT2(CompoundTag aNBT) {
		...
		if (aNBT.contains(NBT_CAPACITY_HU)) mMeltingPoint = aNBT.getLong(NBT_CAPACITY_HU).orElse(0L); else mMeltingPoint = (long)(mMaterial.mMeltingPoint * 1.25);
		mMode = aNBT.getByte(NBT_MODE).orElse((byte)0);
		mSealedTime = aNBT.getLong(NBT_PROGRESS).orElse(0L);
		mTank.setPreventDraining(keepsFilter()).setCapacity(aNBT.getLong(NBT_TANK_CAPACITY).orElse(0L)).readFromNBT(aNBT, NBT_TANK);
	}
					...
					if ((mMode & B[1]) != 0) {
						if (mMaxSealedTime <= 0 || mRecipe == null) {
							...
							if (mRecipe != null && mRecipe.mFluidInputs.length > 0 && mRecipe.mFluidOutputs.length > 0 && !FL.gas(mRecipe.mFluidInputs[0]) && !FL.gas(mRecipe.mFluidOutputs[0])) {
								mMaxSealedTime = UT.Code.divup(Math.max(1, mRecipe.getAbsoluteTotalPower()) * Math.max(1, mTank.amount()), Math.max(1, mRecipe.mFluidInputs[0].getAmount()));
							} else {
								mMaxSealedTime = 0;
								mSealedTime = 0;
								mRecipe = null;
							...
							if (mSealedTime < mMaxSealedTime) {
								...
								mTank.setFluid(FL.mul(mRecipe.mFluidOutputs[0], mTank.amount(), mRecipe.mFluidInputs[0].getAmount(), F));
								mMaxSealedTime = 0;
								mSealedTime = 0;
								mRecipe = null;
							}
