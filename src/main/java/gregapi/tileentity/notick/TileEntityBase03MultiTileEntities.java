...
public abstract class TileEntityBase03MultiTileEntities extends TileEntityBase02Sync implements IRenderedBlockObjectSideCheck, IRenderedBlockObject, IMTE_OnPainting, IMTE_GetPickBlock, IMTE_GetStackFromBlock, IMTE_OnRegistrationFirst, IMTE_RecolourBlock, IMTE_GetDrops, IMTE_OnBlockActivated, IMTE_ShouldSideBeRendered {
	...
	public final void readFromNBT(CompoundTag aNBT) {
		...
		if (WD.tileYInvalid(getLevel(), getBlockPos().getY())) WD.invalidateTileEntityWithNegativeYCoord(getBlockPos().getX(), getBlockPos().getY(), getBlockPos().getZ(), this); // was Y<0; threshold is now the world floor getMinY() (bedrock at Y=-64 is legitimate; notick-MTE: stones/sticks)
	...
	/** The non-ticking MTE hierarchy carries the same two identity fields through the same channel as the ticking one. */
