...
public abstract class MultiTileEntitySensorTE extends MultiTileEntitySensor implements ITileEntityComputerizable, ITileEntityServerTickPost {
	...
	public long onToolClick2(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer, List<String> aChatReturn, Container aPlayerInventory, boolean aSneaking, ItemStack aStack, byte aSide, float aHitX, float aHitY, float aHitZ) {
		...
		if (aTool.equals(TOOL_screwdriver)) {
			...
			float[] tCoords = UT.Code.getFacingCoordsClicked(aSide, aHitX, aHitY, aHitZ);
			if (tCoords[1] >= PX_P[ 6] && tCoords[1] <= PX_P[ 8]) {
				if (tCoords[0] >= PX_P[ 9] && tCoords[0] <= PX_P[11]) {if (mMode < 0) mValues = new int[(int)UT.Code.bind_(1, MAX_AVERAGING_VALUES, mValues.length - 256)]; else mValues = new int[(int)UT.Code.bind_(1, MAX_AVERAGING_VALUES, mValues.length - 100)]; playClick(); if (aChatReturn != null) aChatReturn.add(LH.tt("Averaging over: ") + mValues.length + LH.tt(" Values")); return 1000;}
				if (tCoords[0] >= PX_P[12] && tCoords[0] <= PX_P[14]) {if (mMode < 0) mValues = new int[(int)UT.Code.bind_(1, MAX_AVERAGING_VALUES, mValues.length + 256)]; else mValues = new int[(int)UT.Code.bind_(1, MAX_AVERAGING_VALUES, mValues.length + 100)]; playClick(); if (aChatReturn != null) aChatReturn.add(LH.tt("Averaging over: ") + mValues.length + LH.tt(" Values")); return 1000;}
			}
			if (tCoords[1] >= PX_P[ 9] && tCoords[1] <= PX_P[11]) {
				if (tCoords[0] >= PX_P[ 9] && tCoords[0] <= PX_P[11]) {if (mMode < 0) mValues = new int[(int)UT.Code.bind_(1, MAX_AVERAGING_VALUES, mValues.length - 16)]; else mValues = new int[(int)UT.Code.bind_(1, MAX_AVERAGING_VALUES, mValues.length - 10)]; playClick(); if (aChatReturn != null) aChatReturn.add(LH.tt("Averaging over: ") + mValues.length + LH.tt(" Values")); return 1000;}
				if (tCoords[0] >= PX_P[12] && tCoords[0] <= PX_P[14]) {if (mMode < 0) mValues = new int[(int)UT.Code.bind_(1, MAX_AVERAGING_VALUES, mValues.length + 16)]; else mValues = new int[(int)UT.Code.bind_(1, MAX_AVERAGING_VALUES, mValues.length + 10)]; playClick(); if (aChatReturn != null) aChatReturn.add(LH.tt("Averaging over: ") + mValues.length + LH.tt(" Values")); return 1000;}
			}
			if (tCoords[1] >= PX_P[12] && tCoords[1] <= PX_P[14]) {
				if (tCoords[0] >= PX_P[ 9] && tCoords[0] <= PX_P[11]) {mValues = new int[(int)UT.Code.bind_(1, MAX_AVERAGING_VALUES, mValues.length - 1)]; playClick(); if (aChatReturn != null) aChatReturn.add(mValues.length < 2 ? LH.tt("Averaging Disabled") : LH.tt("Averaging over: ") + mValues.length + LH.tt(" Values")); return 1000;}
				if (tCoords[0] >= PX_P[12] && tCoords[0] <= PX_P[14]) {mValues = new int[(int)UT.Code.bind_(1, MAX_AVERAGING_VALUES, mValues.length + 1)]; playClick(); if (aChatReturn != null) aChatReturn.add(mValues.length < 2 ? LH.tt("Averaging Disabled") : LH.tt("Averaging over: ") + mValues.length + LH.tt(" Values")); return 1000;}
			}
