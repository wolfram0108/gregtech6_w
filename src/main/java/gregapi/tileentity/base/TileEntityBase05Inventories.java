...
public abstract class TileEntityBase05Inventories extends TileEntityBase04MultiTileEntities implements Container, ITileEntityInventoryGUI, IMTE_OnBlockExploded, IMTE_BreakBlock {
	...
	public void readFromNBT2(CompoundTag aNBT) {
		...
		if (mInventory != null && mInventory.length > 0) {
			ListTag tList = aNBT.getList(NBT_INV_LIST, 10);
			for (int i = 0; i < tList.size(); i++) {
				CompoundTag tNBT = tList.getCompound(i);
	...
	// The block itself owns that moment now, via one single bridge in MultiTileEntityBlock.onRemove.
	// A second bridge here would double the drop.
