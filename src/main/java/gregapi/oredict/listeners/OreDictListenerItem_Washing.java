...
public class OreDictListenerItem_Washing extends OreDictListenerItem {
	...
	public ItemStack onTickWorld(OreDictPrefix aPrefix, OreDictMaterial aMaterial, ItemStack aStack, ItemEntity aItem) {
		if (aMaterial != null) {
			...
			// The water cauldron's fill level is now a BlockState LEVEL property, not metadata, so it is read from the
			// block state instead.
			...
			if (tBlock instanceof net.minecraft.world.level.block.LayeredCauldronBlock && tCauldronState.getValue(net.minecraft.world.level.block.LayeredCauldronBlock.LEVEL) > 0) {
				...
				if (tStack != null) {
					net.minecraft.world.level.block.LayeredCauldronBlock.lowerFillLevel(tCauldronState, aItem.level(), tCauldronPos); // Decrements the cauldron's level property by one, replacing the removed metadata setter.
					...
					aItem.setDeltaMovement(0, 0, 0); // Replaces the removed motion fields with setDeltaMovement(0,0,0).
