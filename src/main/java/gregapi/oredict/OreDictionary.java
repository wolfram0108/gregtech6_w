...
public class OreDictionary {
	 ...
	 *
	 * <p><b>Список 1.7.10 продолжен новым ванильным содержимым, а не переосмыслен.</b> Forge-список
	 ...
	 * остаётся замороженным на 2014 годе. Правило добавления жёсткое и проверяемое: (а) имя берётся ТОЛЬКО у
	 * Грега — либо это уже существующее имя семьи из списка выше (новый член вписывается в него), либо его
	 * собственное правило {@code префикс + mNameInternal материала} при УЖЕ существующих
	 * {@link gregapi.data.OP}-префиксе и {@link gregapi.data.MT}-материале; (б) новых префиксов и материалов не
	 * заводится; (в) ванильная медь сюда не попадает — её ведёт отдельная загрузка ванильных рецептов. Ванильные
	 * сущности, для которых у Грега имени НЕТ (смола {@code resin_clump} и её блок, осколок и кристаллы
	 * призмарина, почкующийся аметист и друза, соты, эхо-осколок, окисленные и вощёные формы меди) либо чья
	 * ёмкость его префиксу противоречит ({@code amethyst_block} — четыре осколка против девяти у {@code
	 * blockGem}), не регистрируются вовсе — выдумывать имя запрещено.</p>
	 *
	...
	public static void initVanillaEntries() {
		...
		// РАСЩЕПЛЁННЫЕ СЕМЬИ РУД (движок, а не мод): в 1.7.10 у каждой руды был ОДИН блок, в 1.20.1 их несколько,
		...
		// и к брёвнам/доскам/листве выше, где 1.7.10 обходился одной wildcard-записью.
		// У префикса OP.ore своя ветка разбора: она НЕ ставит цель унификации, а лишь выдаёт паспорт
		// (OreDictManager.java addItemData_), поэтому состав целей этими строками не меняется.
		...
		// Что руда/самородок/слиток ИМЕННО этого материала — незеритовое семейство, Грег говорит сам
		...
		// НОВЫЕ ванильные блоки-хранилища — продолжение ровно этого списка. Имена — то же правило
		// «префикс + материал»: blockNetherite и blockRaw<Материал> уже живут в языке Грега как ключи его же
		// пере-регистраций (LoaderOreDictReRegistrations.java blockNetherite -> blockIngotNetherite;
		// OP.java blockRaw = «New Vanilla Storage Block for Ores» — префикс заведён Грегом ИМЕННО под этот
		...
		// (recipe/amethyst_block.json), а blockGem у Грега — девять; имя blockAmethyst разбирается в
		// blockGem+Amethyst и объявило бы неверную массу, попутно перехватив цель унификации грегского
		// «Block of Amethyst». Соответствия у Грега для «блока из четырёх» нет — не заводим.
		...
		// НОВЫЕ ванильные предметы-материалы — продолжение того же списка теми же именами Грега:
		// nuggetIron рядом с nuggetGold, gemAmethyst рядом с gemDiamond/gemEmerald/gemQuartz, ingotNetherite
		// (ключ пере-регистрации LoaderOreDictReRegistrations.java), «Netherite Scrap» = ингот материала
		// AncientDebris (LanguageHandler.java — Грег сам зовёт этим именем ИМЕННО ингот этого материала),
		// oreRaw<Материал> для сырой руды (OP.java — «The Vanilla "Raw Ore" Item from the Future»).
		// Медь в этот список НЕ входит намеренно: ванильное медное семейство ведёт отдельная загрузка
		// ванильных рецептов, СОЗНАТЕЛЬНО оставляющая целью унификации грегский слиток; дублировать здесь
		// значило бы перехватить паспорта первым.
