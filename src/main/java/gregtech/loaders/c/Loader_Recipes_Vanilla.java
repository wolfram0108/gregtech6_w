...
public class Loader_Recipes_Vanilla implements Runnable {
	@Override public void run() {
		...
		RM.rem_smelting(ST.make(Items.INK_SAC, 1, 0), ST.make(Items.SLIME_BALL, 1, W));
		
		CR.remove(ST.make(Items.SUGAR_CANE, 1, 0));
		CR.remove(ST.make(Items.SUGAR_CANE, 1, 0), ST.make(Items.SUGAR_CANE, 1, 0), ST.make(Items.SUGAR_CANE, 1, 0));
		...
		CR.shaped(ST.make(Items.PAPER, 1, 0), DEF, "XXX", 'X', Items.SUGAR_CANE);
		...
		CR.shaped(ST.make(Blocks.OAK_PRESSURE_PLATE, 1, 0)   , DEF, "PP", 'P', OD.plankAnyWood);
		...
		CR.shaped(ST.make(Blocks.QUARTZ_SLAB, 1, 0), DEF    ,                "XX", 'X', Items.QUARTZ);
		...
		CR.shaped(ST.make(Blocks.CHISELED_STONE_BRICKS, 1, 0), DEF, "f", "X", 'X', ST.make(Blocks.STONE_SLAB, 1, 8));
		
		// Slab Stacking back to full Blocks.
		CR.shaped(ST.make(Blocks.STONE_SLAB          , 1, 0), DEF, "B", "B", 'B', ST.make(Blocks.STONE_SLAB, 1, 0));
		CR.shaped(ST.make(Blocks.COBBLESTONE                , 1, 0), DEF, "B", "B", 'B', ST.make(Blocks.COBBLESTONE_SLAB, 1, 0));
		CR.shaped(ST.make(Blocks.BRICKS                , 1, 0), DEF, "B", "B", 'B', ST.make(Blocks.BRICK_SLAB, 1, 0));
		CR.shaped(ST.make(Blocks.STONE_BRICKS, 1, 0), DEF, "B", "B", 'B', ST.make(Blocks.STONE_BRICK_SLAB, 1, 0));
		CR.shaped(ST.make(Blocks.NETHER_BRICKS               , 1, 0), DEF, "B", "B", 'B', ST.make(Blocks.NETHER_BRICK_SLAB, 1, 0));
		CR.shaped(ST.make(Blocks.QUARTZ_BLOCK, 1, 0), DEF, "B", "B", 'B', ST.make(Blocks.QUARTZ_SLAB, 1, 0));
		CR.shaped(ST.make(Blocks.STONE_SLAB          , 1, 8), DEF, "B", "B", 'B', ST.make(Blocks.STONE_SLAB, 1, 8));
		...
		CR.shapeless(ST.make(Blocks.STONE_SLAB       , 1, 8), DEF, new Object[] {ST.make(Blocks.STONE_SLAB, 1, 0)});
		CR.shapeless(ST.make(Blocks.STONE_SLAB       , 1, 0), DEF, new Object[] {ST.make(Blocks.STONE_SLAB, 1, 8)});
		
		// Make Sticks from Saplings and Dead Bushes.
		CR.shaped(stick.mat(MT.WOODS.Dead, 2), DEF, "s", "X", 'X', ST.make(Blocks.DEAD_BUSH, 1, W));
		CR.shaped(stick.mat(MT.WOODS.Dead, 2), DEF, "k", "X", 'X', ST.make(Blocks.DEAD_BUSH, 1, W));
		CR.shaped(stick.mat(MT.WOODS.Dead, 2), DEF, "s", "X", 'X', ST.make(Blocks.DEAD_BUSH, 1, 0));
		CR.shaped(stick.mat(MT.WOODS.Dead, 2), DEF, "k", "X", 'X', ST.make(Blocks.DEAD_BUSH, 1, 0));
		...
		for (int i = 0; i < tItems.length; i++) if (tItems[i].exists()) {
			...
			CR.remove(ST.make(Items.CHARCOAL, 1, 0), NI, NI, ST.make(Items.STICK, 1, 0));
			} else {
		...
		CR.shaped(ST.make(Items.CHARCOAL, 1, 0), DEF, "  ", " X", 'X', ingot.dat(MT.Charcoal));
		CR.shaped(ingot.mat(MT.Charcoal, 1), DEF, "  ", " X", 'X', ST.make(Items.CHARCOAL, 1, 0));
		RM.generify(ingot.mat(MT.Charcoal, 1), ST.make(Items.CHARCOAL, 1, 0));
		RM.generify(ST.make(Items.CHARCOAL, 1, 0), ingot.mat(MT.Charcoal, 1));
		...
		CR.delate(ST.make(Items.ENCHANTED_GOLDEN_APPLE, 1, 0));
		...
		CR.shaped(ST.make(Items.GLISTERING_MELON_SLICE, 1, 0), DEF | DEL_OTHER_SHAPED_RECIPES, "GGG", "GMG", "GGG", 'M', "cropMelon", 'G', nugget.dat(MT.Au));
		...
		RM.smash(ST.make(Blocks.STONE_SLAB    , 1, 7), gem.mat(MT.NetherQuartz, 4));
		RM.smash(ST.make(Blocks.QUARTZ_STAIRS        , 1, W), gem.mat(MT.NetherQuartz, 6));
		RM.smash(ST.make(Blocks.QUARTZ_SLAB, 1, 0), gem.mat(MT.NetherQuartz, 2));
		RM.smash(ST.make(Blocks.STONE_BRICKS, 1, 0), ST.make(Blocks.CRACKED_STONE_BRICKS, 1, 0));
		RM.smash(ST.make(Blocks.MOSSY_STONE_BRICKS, 1, 0), ST.make(Blocks.COBBLESTONE, 1, 0));
		RM.smash(ST.make(Blocks.CRACKED_STONE_BRICKS, 1, 0), ST.make(Blocks.COBBLESTONE, 1, 0));
		RM.smash(ST.make(Blocks.CHISELED_STONE_BRICKS, 1, 0), ST.make(Blocks.COBBLESTONE, 1, 0));
		...
		RM.smash(ST.make(Blocks.WHITE_STAINED_GLASS, 1, W), OM.dust(MT.Glass, U*9));
		RM.smash(ST.make(Blocks.GLASS_PANE           , 1, W), OM.dust(MT.Glass, U));
		RM.smash(ST.make(Blocks.WHITE_STAINED_GLASS_PANE, 1, W), OM.dust(MT.Glass, U));
		
		RM.Hammer .addRecipe1(T, 16,  16, ST.make(Blocks.BRICKS        , 1, W), ST.make(Items.BRICK, 3, 0));
		RM.Hammer .addRecipe1(T, 16,  16, ST.make(Blocks.STONE_SLAB  , 1, 4), ST.make(Items.BRICK, 3, 0));
		RM.Hammer .addRecipe1(T, 16,  16, ST.make(Blocks.BRICK_STAIRS       , 1, W), ST.make(Items.BRICK, 2, 0));
		RM.Hammer .addRecipe1(T, 16,  16, ST.make(Blocks.BRICK_SLAB, 1, 0), ST.make(Items.BRICK, 1, 0));
		RM.Crusher.addRecipe1(T, 16,  64, new long[] {10000, 9000, 8000, 7000}, ST.make(Blocks.BRICKS        , 1, W), ST.make(Items.BRICK, 1, 0), ST.make(Items.BRICK, 1, 0), ST.make(Items.BRICK, 1, 0), ST.make(Items.BRICK, 1, 0));
		RM.Crusher.addRecipe1(T, 16,  64, new long[] {10000, 9000, 8000, 7000}, ST.make(Blocks.STONE_SLAB  , 1, 4), ST.make(Items.BRICK, 1, 0), ST.make(Items.BRICK, 1, 0), ST.make(Items.BRICK, 1, 0), ST.make(Items.BRICK, 1, 0));
		...
		RM.Crusher.addRecipe1(T, 16,  32, new long[] {10000, 8000            }, ST.make(Blocks.BRICK_SLAB, 1, 0), ST.make(Items.BRICK, 1, 0), ST.make(Items.BRICK, 1, 0));
		
		RM.Hammer .addRecipe1(T, 16,  16, ST.make(Blocks.NETHER_BRICKS       , 1, W), ST.make(Items.NETHER_BRICK, 3, 0));
		RM.Hammer .addRecipe1(T, 16,  16, ST.make(Blocks.STONE_SLAB  , 1, 6), ST.make(Items.NETHER_BRICK, 3, 0));
		RM.Hammer .addRecipe1(T, 16,  16, ST.make(Blocks.NETHER_BRICK_STAIRS, 1, W), ST.make(Items.NETHER_BRICK, 2, 0));
		RM.Hammer .addRecipe1(T, 16,  16, ST.make(Blocks.NETHER_BRICK_FENCE , 1, W), ST.make(Items.NETHER_BRICK, 2, 0));
		RM.Hammer .addRecipe1(T, 16,  16, ST.make(Blocks.NETHER_BRICK_SLAB, 1, 0), ST.make(Items.NETHER_BRICK, 1, 0));
		RM.Crusher.addRecipe1(T, 16,  64, new long[] {10000, 9000, 8000, 7000}, ST.make(Blocks.NETHER_BRICKS       , 1, W), ST.make(Items.NETHER_BRICK, 1, 0), ST.make(Items.NETHER_BRICK, 1, 0), ST.make(Items.NETHER_BRICK, 1, 0), ST.make(Items.NETHER_BRICK, 1, 0));
		RM.Crusher.addRecipe1(T, 16,  64, new long[] {10000, 9000, 8000, 7000}, ST.make(Blocks.STONE_SLAB  , 1, 6), ST.make(Items.NETHER_BRICK, 1, 0), ST.make(Items.NETHER_BRICK, 1, 0), ST.make(Items.NETHER_BRICK, 1, 0), ST.make(Items.NETHER_BRICK, 1, 0));
		RM.Crusher.addRecipe1(T, 16,  64, new long[] {10000, 9000, 8000      }, ST.make(Blocks.NETHER_BRICK_STAIRS, 1, W), ST.make(Items.NETHER_BRICK, 1, 0), ST.make(Items.NETHER_BRICK, 1, 0), ST.make(Items.NETHER_BRICK, 1, 0));
		RM.Crusher.addRecipe1(T, 16,  64, new long[] {10000, 9000, 8000      }, ST.make(Blocks.NETHER_BRICK_FENCE , 1, W), ST.make(Items.NETHER_BRICK, 1, 0), ST.make(Items.NETHER_BRICK, 1, 0), ST.make(Items.NETHER_BRICK, 1, 0));
		RM.Crusher.addRecipe1(T, 16,  32, new long[] {10000, 8000            }, ST.make(Blocks.NETHER_BRICK_SLAB, 1, 0), ST.make(Items.NETHER_BRICK, 1, 0), ST.make(Items.NETHER_BRICK, 1, 0));
		...
		RM.moss    (ST.make(Blocks.STONE_BRICKS, 1, 0), ST.make(Blocks.MOSSY_STONE_BRICKS, 1, 0));
		RM.growmoss(ST.make(Blocks.CRACKED_STONE_BRICKS, 1, 0), ST.make(Blocks.MOSSY_STONE_BRICKS, 1, 0));
		...
		for (byte i = 0; i < 16; i++) {
		RM.sawing(16,  32, F,  50, ST.make(Blocks.WHITE_STAINED_GLASS             , 1, i), ST.make(Blocks.WHITE_STAINED_GLASS_PANE, 9, i));
		RM.sawing(16,  32, F,  50, ST.make(Blocks.WHITE_WOOL                      , 2, i), ST.make(Blocks.WHITE_CARPET, 3, i));
		}
		RM.sawing(16,  32, F,  50, ST.make(Blocks.GLASS                     , 1, 0), ST.make(Blocks.GLASS_PANE, 9, 0));
		RM.sawing(16,  16, F, 100, ST.make(Blocks.STONE_SLAB         , 1, 0), plate.mat(MT.Stone, 8), dust.mat(MT.Stone, 1));
		RM.sawing(16,  16, F, 100, ST.make(Blocks.STONE_SLAB         , 1, 8), plate.mat(MT.Stone, 8), dust.mat(MT.Stone, 1));
		...
		RM.sawing(16,  16, F, 100, ST.make(Blocks.SANDSTONE, 1, 0), ST.make(Blocks.SANDSTONE_SLAB, 2, 0));
		RM.sawing(16,  16, F, 100, ST.make(Blocks.COBBLESTONE               , 1, 0), ST.make(Blocks.COBBLESTONE_SLAB, 2, 0));
		RM.sawing(16,  16, F, 100, ST.make(Blocks.BRICKS               , 1, 0), ST.make(Blocks.BRICK_SLAB, 2, 0));
		RM.sawing(16,  16, F, 100, ST.make(Blocks.STONE_BRICKS, 1, 0), ST.make(Blocks.STONE_BRICK_SLAB, 2, 0));
		RM.sawing(16,  16, F, 100, ST.make(Blocks.NETHER_BRICKS              , 1, 0), ST.make(Blocks.NETHER_BRICK_SLAB, 2, 0));
		RM.sawing(16,  16, F, 100, ST.make(Blocks.QUARTZ_BLOCK, 1, W), ST.make(Blocks.QUARTZ_SLAB, 2, 0));
		RM.sawing(16,  16, F, 100, ST.make(Blocks.OAK_BUTTON             , 1, W), IL.Plank.get(1));
		RM.sawing(16,  32, F, 100, ST.make(Blocks.OAK_PRESSURE_PLATE     , 1, W), IL.Plank.get(2));
		RM.sawing(16,  32, F, 100, ST.make(Items.OAK_SIGN                       , 1, W), IL.Plank.get(2), OM.dust(MT.Wood, stick.mAmount / 3));
		RM.sawing(16,  32, F, 100, ST.make(Items.OAK_DOOR                , 1, W), IL.Plank.get(2));
		RM.sawing(16,  32, F, 100, ST.make(Blocks.OAK_FENCE_GATE                , 1, W), IL.Plank.get(2), OM.dust(MT.Wood, stick.mAmount * 4));
		RM.sawing(16,  48, F, 100, ST.make(Blocks.OAK_TRAPDOOR                  , 1, W), IL.Plank.get(3));
		RM.sawing(16,  48, F, 100, ST.make(Items.WHITE_BED                        , 1, W), IL.Plank.get(3), ST.make(Blocks.WHITE_WOOL, 3, 0));
		...
		RM.sawing(16,  16, T,  50, ST.make(Blocks.MELON               , 1, W), ST.make(Items.MELON_SLICE, 8, 0), ST.make(Items.MELON_SEEDS, 1, 0));
		
		CR.shapeless(IL.Plank.get(1), CR.DEF_NCC, new Object[] {OreDictToolNames.saw, Blocks.OAK_BUTTON        });
		CR.shapeless(IL.Plank.get(2), CR.DEF_NCC, new Object[] {OreDictToolNames.saw, Blocks.OAK_PRESSURE_PLATE});
		CR.shapeless(IL.Plank.get(2), CR.DEF_NCC, new Object[] {OreDictToolNames.saw, Items.OAK_SIGN                  });
		CR.shapeless(IL.Plank.get(2), CR.DEF_NCC, new Object[] {OreDictToolNames.saw, Items.OAK_DOOR           });
		CR.shapeless(IL.Plank.get(2), CR.DEF_NCC, new Object[] {OreDictToolNames.saw, Blocks.OAK_FENCE_GATE           });
		CR.shapeless(IL.Plank.get(3), CR.DEF_NCC, new Object[] {OreDictToolNames.saw, Blocks.OAK_TRAPDOOR             });
		CR.shapeless(IL.Plank.get(3), CR.DEF_NCC, new Object[] {OreDictToolNames.saw, Items.WHITE_BED                   });
		...
		RM.Slicer       .addRecipe2(T, 16,   16, ST.make(Blocks.MELON      , 1, W), IL.Shape_Slicer_Eigths.get(0), ST.make(Items.MELON_SLICE, 8, 0), ST.make(Items.MELON_SEEDS, 1, 0));
		...
		RM.Freezer      .addRecipe1(T, 16,   32*tMul, ST.tag(2), FL.mul(tWater, 2), NF, ST.make(Blocks.SNOW, 1, 0));
		...
		RM.Mortar       .addRecipe1(T, 16, 32, ST.make(Blocks.WHITE_STAINED_GLASS, 1, W), OM.dust(MT.Glass, U*9));
		RM.Mortar       .addRecipe1(T, 16, 32, ST.make(Blocks.GLASS_PANE        , 1, W), OM.dust(MT.Glass, U));
		RM.Mortar       .addRecipe1(T, 16, 32, ST.make(Blocks.WHITE_STAINED_GLASS_PANE, 1, W), OM.dust(MT.Glass, U));
		...
		RM.Mortar       .addRecipe1(T, 16, 16, ST.make(Items.CHARCOAL, 1, 0), OM.dust(MT.Charcoal));
		...
		RM.Shredder     .addRecipe1(T, 16, 16, ST.make(Blocks.COBWEB, 1, W), ST.make(Items.STRING, 1, 0));
		RM.Shredder     .addRecipe1(T, 16, 16, ST.make(Items.SUGAR_CANE, 1, W), IL.Remains_Plant.get(1));
		...
		RM.Bath         .addRecipe1(T,  0,   16, ST.make(Blocks.WHITE_CARPET, 1, 15-i), MT.Cl.fluid(U40, T), NF, ST.make(Blocks.WHITE_CARPET, 1, 0));
		}
		...
		RM.Bath         .addRecipe1(T,  0,   16, ST.make(Blocks.WHITE_CARPET, 1, 0), FL.mul(tDye, 1, 32, T), NF, ST.make(Blocks.WHITE_CARPET               , 1, 15-i));
		}
		RM.Bath         .addRecipe1(T,  0,   16, ST.make(Blocks.TERRACOTTA, 1, 0), FL.mul(tDye, 1, 16, T), NF, ST.make(Blocks.WHITE_TERRACOTTA, 1, 15-i));
		RM.Bath         .addRecipe1(T,  0,   16, ST.make(Blocks.GLASS        , 1, 0), FL.mul(tDye, 1, 16, T), NF, ST.make(Blocks.WHITE_STAINED_GLASS        , 1, 15-i));
		RM.Bath         .addRecipe1(T,  0,   16, ST.make(Blocks.GLASS_PANE   , 1, 0), FL.mul(tDye, 3,128, T), NF, ST.make(Blocks.WHITE_STAINED_GLASS_PANE   , 1, 15-i));
		}
		...
		RM.Bath         .addRecipe1(T,  0,   16, ST.make(Blocks.WHITE_STAINED_GLASS, 1, W), MT.Cl.fluid(U20, T), NF, ST.make(Blocks.GLASS        , 1, 0));
		RM.Bath         .addRecipe1(T,  0,   16, ST.make(Blocks.WHITE_STAINED_GLASS_PANE, 1, W), MT.Cl.fluid(U50, T), NF, ST.make(Blocks.GLASS_PANE   , 1, 0));
		...
		RM.Loom         .addRecipe2(T, 16,   64, ST.tag(1), ST.make(Items.STRING, 4, W), ST.make(Blocks.COBWEB, 1, 0));
		RM.Loom         .addRecipe2(T, 16,   16, ST.tag(0), ST.make(Items.SUGAR_CANE, 1, W), ST.make(Items.PAPER, 1, 0));
		for (FluidStack tWater : FL.waters(125, 100))
		RM.Bath         .addRecipe1(T,  0,   16, ST.make(Items.SUGAR_CANE, 1, W), tWater, NF, ST.make(Items.PAPER, 1, 0));
		...
		RM.Chisel       .addRecipe1(T, 16,   16, ST.make(Blocks.STONE, 1, W), ST.make(Blocks.CHISELED_STONE_BRICKS, 1, 0));
		RM.Chisel       .addRecipe1(T, 16,   16, ST.make(Blocks.STONE_BRICKS, 1, 0), ST.make(Blocks.CRACKED_STONE_BRICKS, 1, 0));
		...
		RM.Squeezer     .addRecipe1(T, 16,   16,  2000, ST.make(Blocks.POPPY, 1, 0), NF, FL.mul(DYE_FLUIDS_FLOWER[DYE_INDEX_Red          ], 2), IL.Remains_Plant.get(1));
		RM.Squeezer     .addRecipe1(T, 16,   16,  2000, ST.make(Blocks.BLUE_ORCHID, 1, 0), NF, FL.mul(DYE_FLUIDS_FLOWER[DYE_INDEX_LightBlue    ], 2), IL.Remains_Plant.get(1));
		RM.Squeezer     .addRecipe1(T, 16,   16,  2000, ST.make(Blocks.ALLIUM, 1, 0), NF, FL.mul(DYE_FLUIDS_FLOWER[DYE_INDEX_Magenta      ], 2), IL.Remains_Plant.get(1));
		RM.Squeezer     .addRecipe1(T, 16,   16,  2000, ST.make(Blocks.AZURE_BLUET, 1, 0), NF, FL.mul(DYE_FLUIDS_FLOWER[DYE_INDEX_LightGray    ], 2), IL.Remains_Plant.get(1));
		RM.Squeezer     .addRecipe1(T, 16,   16,  2000, ST.make(Blocks.RED_TULIP, 1, 0), NF, FL.mul(DYE_FLUIDS_FLOWER[DYE_INDEX_Red          ], 2), IL.Remains_Plant.get(1));
		RM.Squeezer     .addRecipe1(T, 16,   16,  2000, ST.make(Blocks.ORANGE_TULIP, 1, 0), NF, FL.mul(DYE_FLUIDS_FLOWER[DYE_INDEX_Orange       ], 2), IL.Remains_Plant.get(1));
		RM.Squeezer     .addRecipe1(T, 16,   16,  2000, ST.make(Blocks.WHITE_TULIP, 1, 0), NF, FL.mul(DYE_FLUIDS_FLOWER[DYE_INDEX_LightGray    ], 2), IL.Remains_Plant.get(1));
		RM.Squeezer     .addRecipe1(T, 16,   16,  2000, ST.make(Blocks.PINK_TULIP, 1, 0), NF, FL.mul(DYE_FLUIDS_FLOWER[DYE_INDEX_Pink         ], 2), IL.Remains_Plant.get(1));
		RM.Squeezer     .addRecipe1(T, 16,   16,  2000, ST.make(Blocks.OXEYE_DAISY, 1, 0), NF, FL.mul(DYE_FLUIDS_FLOWER[DYE_INDEX_LightGray    ], 2), IL.Remains_Plant.get(1));
		RM.Squeezer     .addRecipe1(T, 16,   16,  2000, ST.make(Blocks.DANDELION, 1, 0), NF, FL.mul(DYE_FLUIDS_FLOWER[DYE_INDEX_Yellow       ], 2), IL.Remains_Plant.get(1));
		...
		RM.Squeezer     .addRecipe1(T, 16,   16,  4000, ST.make(Items.SUGAR_CANE             , 1, W), NF, FL.Juice_Reed.make(100), IL.Remains_Plant.get(1));
		...
		RM.Squeezer     .addRecipe1(T, 16,   32, 10000, ST.make(Items.PUFFERFISH, 1, 0), NF, FL.Potion_Poison_2.make(250), OM.dust(MT.FishRaw, U));
		RM.Squeezer     .addRecipe1(T, 16,  128, 10000, ST.make(Items.ENCHANTED_GOLDEN_APPLE, 1, 0), NF, FL.make("potion.idunsapplejuice", 250), OM.dust(MT.Au, U*18));
		...
		RM.Juicer       .addRecipe1(T, 16,   16,  3000, ST.make(Blocks.POPPY, 1, 0), NF, FL.mul(DYE_FLUIDS_FLOWER[DYE_INDEX_Red          ], 1), IL.Remains_Plant.get(1));
		RM.Juicer       .addRecipe1(T, 16,   16,  3000, ST.make(Blocks.BLUE_ORCHID, 1, 0), NF, FL.mul(DYE_FLUIDS_FLOWER[DYE_INDEX_LightBlue    ], 1), IL.Remains_Plant.get(1));
		RM.Juicer       .addRecipe1(T, 16,   16,  3000, ST.make(Blocks.ALLIUM, 1, 0), NF, FL.mul(DYE_FLUIDS_FLOWER[DYE_INDEX_Magenta      ], 1), IL.Remains_Plant.get(1));
		RM.Juicer       .addRecipe1(T, 16,   16,  3000, ST.make(Blocks.AZURE_BLUET, 1, 0), NF, FL.mul(DYE_FLUIDS_FLOWER[DYE_INDEX_LightGray    ], 1), IL.Remains_Plant.get(1));
		RM.Juicer       .addRecipe1(T, 16,   16,  3000, ST.make(Blocks.RED_TULIP, 1, 0), NF, FL.mul(DYE_FLUIDS_FLOWER[DYE_INDEX_Red          ], 1), IL.Remains_Plant.get(1));
		RM.Juicer       .addRecipe1(T, 16,   16,  3000, ST.make(Blocks.ORANGE_TULIP, 1, 0), NF, FL.mul(DYE_FLUIDS_FLOWER[DYE_INDEX_Orange       ], 1), IL.Remains_Plant.get(1));
		RM.Juicer       .addRecipe1(T, 16,   16,  3000, ST.make(Blocks.WHITE_TULIP, 1, 0), NF, FL.mul(DYE_FLUIDS_FLOWER[DYE_INDEX_LightGray    ], 1), IL.Remains_Plant.get(1));
		RM.Juicer       .addRecipe1(T, 16,   16,  3000, ST.make(Blocks.PINK_TULIP, 1, 0), NF, FL.mul(DYE_FLUIDS_FLOWER[DYE_INDEX_Pink         ], 1), IL.Remains_Plant.get(1));
		RM.Juicer       .addRecipe1(T, 16,   16,  3000, ST.make(Blocks.OXEYE_DAISY, 1, 0), NF, FL.mul(DYE_FLUIDS_FLOWER[DYE_INDEX_LightGray    ], 1), IL.Remains_Plant.get(1));
		RM.Juicer       .addRecipe1(T, 16,   16,  3000, ST.make(Blocks.DANDELION, 1, 0), NF, FL.mul(DYE_FLUIDS_FLOWER[DYE_INDEX_Yellow       ], 1), IL.Remains_Plant.get(1));
		...
		RM.Juicer       .addRecipe1(T, 16,   16,  5000, ST.make(Items.SUGAR_CANE             , 1, W), NF, FL.Juice_Reed.make(75), IL.Remains_Plant.get(1));
		...
		RM.Bath         .addRecipe1(T,  0,   16, ST.make(Items.MELON_SLICE        , 1, W), MT.Au.liquid(8*U9, T), NF, ST.make(Items.GLISTERING_MELON_SLICE, 1, 0));
		RM.Mixer        .addRecipe2(T, 16,   16, ST.make(Items.MELON_SLICE        , 1, W), nugget.mat(MT.Au, 8), ST.make(Items.GLISTERING_MELON_SLICE, 1, 0));
		...
		RM.ic2_compressor(ST.make(Blocks.POPPY, 8, W), IL.IC2_Plantball.get(1));
		RM.ic2_compressor(ST.make(Blocks.DANDELION, 8, W), IL.IC2_Plantball.get(1));
		RM.ic2_compressor(ST.make(Blocks.SUNFLOWER, 8, W), IL.IC2_Plantball.get(1));
		
		RM.pulverizing(ST.make(Items.SUGAR_CANE, 1, W), IL.Remains_Plant.get(1), T);
		...
		RM.biomass(ST.make(Blocks.POPPY, 8, W));
		RM.biomass(ST.make(Blocks.DANDELION, 8, W));
		RM.biomass(ST.make(Blocks.SUNFLOWER, 8, W));
		...
		RM.biomass(ST.make(Items.SUGAR_CANE, 8, W));
		RM.biomass(ST.make(Items.MELON_SLICE, 9, W));
		...
		RM.pack(ST.make(Items.BRICK      , 2, W), ST.make(Blocks.BRICK_SLAB, 1, 0));
		RM.pack(ST.make(Items.BRICK      , 3, W), ST.make(Blocks.BRICK_STAIRS       , 1, 0));
		RM.pack(ST.make(Items.BRICK      , 4, W), ST.make(Blocks.BRICKS        , 1, 0));
		
		RM.pack(ST.make(Items.NETHER_BRICK, 2, W), ST.make(Blocks.NETHER_BRICK_SLAB, 1, 0));
		RM.pack(ST.make(Items.NETHER_BRICK, 3, W), ST.make(Blocks.NETHER_BRICK_STAIRS, 1, 0));
		RM.pack(ST.make(Items.NETHER_BRICK, 4, W), ST.make(Blocks.NETHER_BRICKS       , 1, 0));
	...
	private static void copperAge() {
		...
		// Entry to the copper age: nine GT ingots craft the vanilla copper block.
		// The rest of 26.1.2's copper content follows vanilla paths from there, untouched by us.
		CR.shaped(ST.make(Items.COPPER_BLOCK          , 1, 0), DEF_REM    , "XXX", "XXX", "XXX", 'X', tIngot);

		// Items that vanilla recipes require a copper ingot for.
		...
		// Vanilla copper ingot and its byproducts are never crafted directly: no recipe yields them.
		// (Ore/raw smelting is out of scope here — a datapack smelting recipe, see ADAPT-014 "Not done".)
		CR.delate(ST.make(Items.COPPER_INGOT     , 1, 0));
		CR.delate(ST.make(Items.RAW_COPPER_BLOCK , 1, 0));

		// MATERIAL LEAK: vanilla copper reachable outside these recipes despite the craft ban above —
		// drowned/copper_golem loot, armor/tool smelting (datapack; GT_API.java:659,668 covers only
		// CraftingRecipe) and raw ore blocks all yield vanilla items, sinking already-mined GT copper.
		//
		// Fix: register these items under our material name like any foreign item (cf.
		// OreDictionary.java:196-199); CR.java:401-411 matches recipe ingredients by OreDict name.
		//
		// setTarget_ (OreDictManager.java:471) only fills an empty target slot, but phase order is not
		// guaranteed — hijacking it would redirect ALL GT copper to the vanilla ingot. So the GT stack
		// is read BEFORE registration and restored as target AFTER, with aOverwrite=true.
		// Verified by stand gt6copperprobe §6.
		// ORDER MATTERS: assign material data BEFORE registering names — addItemData_ (OreDictManager:667)
		// won't overwrite an existing association, and name registration creates one. Reversed order left
		// raw_copper associated but data-less (measured: 130 items affected).
		copperItemData();
	...
	private static void copperItemData() {
		...
		tForms.put("copper"              , U*9  ); // oxidized block variants are named exposed_copper etc.
		...
		tForms.put("copper_golem_statue" , U*9  ); // no recipe (structure/creative only); same look as a copper block
		...
		// form's secondary material (stick/plank wood), U2 = half a unit per stick
		...
		// Ore forms are described by PREFIX, not mass: oreVanillastone/oreDeepslate/oreRaw carry
		// setOreStats(2*U) (OP.java:63,67,137) and follow the ore path, not "melt into two ingots".
		...
		for (net.minecraft.world.item.Item tItem : net.minecraft.core.registries.BuiltInRegistries.ITEM) {
			...
			// ore (its own prefix, set separately) and the spawn egg carry no material data
