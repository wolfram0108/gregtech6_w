...
public class WorldgenRocks extends WorldgenOnSurface {
	...
	public boolean tryPlaceStuff(WorldGenLevel aWorld, int aX, int aY, int aZ, Random aRandom, Block aContact) {
		...
		if (WD.easyRepDry(aWorld, aX, aY+1, aZ)) return tRegistry.mBlock.placeBlock(aWorld, aX, aY+1, aZ, SIDE_UNKNOWN, (short)32757, aRandom.nextInt(mAmount) == 0 ? ST.save(NBT_VALUE, aRandom.nextInt(12)==0?(aRandom.nextInt(4)==0?OP.oreRaw:OP.rockGt).mat(MT.MeteoricIron, 1):ST.make(Items.FLINT, 1, 0)) : null, F, T);
		return F;
	}
}
