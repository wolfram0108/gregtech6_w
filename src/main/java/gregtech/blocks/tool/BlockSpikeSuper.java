...
public class BlockSpikeSuper extends BlockBaseSpike {
	...
	public void addInformation(ItemStack aStack, byte aMeta, Player aPlayer, List<String> aList, boolean aF3_H) {
		if (aMeta < 8) {
			aList.add(LH.Chat.ORANGE + LH.tt("Deals huge Damage to anything touching it!"));
		} else {
			aList.add(LH.Chat.ORANGE + LH.tt("Deals absurd amounts of Damage to anything touching it!"));
		}
		if ((aMeta & 7) >= 6) {
	...
	public void onEntityCollidedWithBlock(Level aWorld, int aX, int aY, int aZ, Entity aEntity) {
		...
		if (aEntity instanceof LivingEntity) {
			if (aMeta < 8) {
				aEntity.hurt(DamageSources.getSpikeDamage(), TFC_DAMAGE_MULTIPLIER * ((aMeta & 7) < 6 ? 15.0F :  7.5F));
			} else {
				aEntity.hurt(DamageSources.getSpikeDamage(), TFC_DAMAGE_MULTIPLIER * ((aMeta & 7) < 6 ? 50.0F : 25.0F));
			}
		}
	}
