...
public class BlockSpikeFancy extends BlockBaseSpike {
	...
	public void addInformation(ItemStack aStack, byte aMeta, Player aPlayer, List<String> aList, boolean aF3_H) {
		if (aMeta < 8) {
			aList.add(LH.Chat.ORANGE + LH.tt("Deals huge Damage to any Undead touching it!"));
			aList.add(LH.Chat.ORANGE + LH.tt("Does very low Damage to anything else!"));
			aList.add(LH.Chat.ORANGE + LH.tt("Doesn't work on Slimes and Iron Golems."));
		} else {
			aList.add(LH.Chat.ORANGE + LH.tt("Deals huge Damage to any Enderman, Werewolf or Bear989Sr touching it!"));
			aList.add(LH.Chat.ORANGE + LH.tt("Does very low Damage to anything else!"));
	...
	public void onEntityCollidedWithBlock(Level aWorld, int aX, int aY, int aZ, Entity aEntity) {
		...
		if (aEntity instanceof LivingEntity) {
			if (aMeta < 8) {
				if (((LivingEntity)aEntity).getMobType() == net.minecraft.world.entity.MobType.UNDEAD)
