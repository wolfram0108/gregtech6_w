...
public class BlockDiggable extends BlockBaseMeta implements IBlockOnWalkOver {
	...
	public boolean canSustainPlant(BlockGetter aWorld, int aX, int aY, int aZ, Direction aSide, IPlantable aPlant) {
		...
		if (aPlant == Blocks.SUGAR_CANE || aPlant instanceof BushBlock) return T;
	...
	public void onWalkOver(LivingEntity aEntity, Level aWorld, int aX, int aY, int aZ) {
		if (doesWalkSpeed(WD.meta(aWorld, aX, aY, aZ))) {aEntity.getDeltaMovement().x *= 0.5; aEntity.getDeltaMovement().z *= 0.5;}
	}
	...
	@Override public float getBlockHardness(Level aWorld, int aX, int aY, int aZ) {return WD.hardness(Blocks.DIRT, aWorld, aX, aY, aZ);}
