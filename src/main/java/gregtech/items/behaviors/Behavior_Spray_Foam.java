...
public class Behavior_Spray_Foam extends AbstractBehaviorDefault {
	...
	public boolean onItemUseFirst(MultiItem aItem, ItemStack aStack, Player aPlayer, Level aWorld, int aX, int aY, int aZ, byte aSide, float hitX, float hitY, float hitZ) {
		if (aWorld.isClientSide() || aStack.getCount() != 1 || aPlayer.isShiftKeyDown() || !(aPlayer).mayUseItemAt(new BlockPos(aX, aY, aZ), FORGE_DIR[aSide], aStack)) return F;
	...
	public long foam(Level aWorld, int aX, int aY, int aZ, byte aSide, long aUses, Player aPlayer, ItemStack aStack) {
		...
		if (IL.IC2_Scaffold_Iron.equal(aBlock)) return aUses >= 10 && WD.set(aWorld, aX, aY, aZ, IL.IC2_Foam_Reinforced.block(), 0, 3) ? 10 : 0;
		...
		if (BlocksGT.CFoamFresh != null) {
			...
			case 0:
				if (aUses >= 10 && WD.air(aWorld, aX, aY, aZ) && (mOwned?MultiTileEntityCFoam.setBlock(aWorld, aX, aY, aZ, aSide, aPlayer, aStack, DYES[mColor], mOwned):WD.set(aWorld, aX, aY, aZ, BlocksGT.CFoamFresh, mColor, 3))) {aUses-=10; rUses+=10;}
				return rUses;
			case 1:
				for (byte i = 0; i < 4; i++) {
					if (aUses >= 10 && WD.air(aWorld, aX, aY, aZ) && (mOwned?MultiTileEntityCFoam.setBlock(aWorld, aX, aY, aZ, aSide, aPlayer, aStack, DYES[mColor], mOwned):WD.set(aWorld, aX, aY, aZ, BlocksGT.CFoamFresh, mColor, 3))) {aUses-=10; rUses+=10;} else break;
			...
			case 4:
				...
				for (byte i = 0; i < 3; i++) for (byte j = 0; j < 3; j++) {
					if (aUses >= 5) {
						if (WD.air(aWorld, aX + (SIDES_AXIS_X[tSide]?0:i), aY + (SIDES_AXIS_X[tSide]?i:0) + (SIDES_AXIS_Z[tSide]?j:0), aZ + (SIDES_AXIS_Z[tSide]?0:j))) {
							if (WD.set(aWorld, aX + (SIDES_AXIS_X[tSide]?0:i), aY + (SIDES_AXIS_X[tSide]?i:0) + (SIDES_AXIS_Z[tSide]?j:0), aZ + (SIDES_AXIS_Z[tSide]?0:j), ((BlockCFoamFresh)BlocksGT.CFoamFresh).mSlabs[OPOS[tSide]], mColor, 3)) {aUses-=5; rUses+=5;}
						}
					} else break;
				}
				return rUses;
