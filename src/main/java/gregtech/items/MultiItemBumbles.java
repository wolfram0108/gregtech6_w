...
public class MultiItemBumbles extends MultiItemRandomWithCompat implements IItemBumbleBee {
	...
	public BlockPos bumbleCanProduce(Level aWorld, int aX, int aY, int aZ, ItemStack aBumbleBee, short aMetaData, int aDistance) {
		...
		case   4: case 202:
			...
			if (tChorusFlower == null) {
				if (WD.dimensionId(aWorld) == 1 || BIOMES_END.contains(WD.biome(aWorld, aX, aZ))) return new BlockPos(aX, aY, aZ);
				for (int j : tOrderY) for (int i : tOrderX) for (int k : tOrderZ) {
					Block tBlock = WD.block(aWorld, aX+i, aY+j, aZ+k, F);
	...
	@Override
	// F3 superseded-render (GT6BlockModel/ItemModel pipeline; the old getIcon/immediate-mode is dead, 0 neo calls): was aIconRegister.registerIcon(...) (IIconRegister removed) — we build the Identifier directly from the same path.
