...
public class GT_Tool_Axe extends ToolStats {
	...
	@Override
	public int convertBlockDrops(List<ItemStack> aDrops, ItemStack aStack, Player aPlayer, Block aBlock, long aAvailableDurability, int aX, int aY, int aZ, byte aMeta, int aFortune, boolean aSilkTouch) {
		int rAmount = 0;
		if (LOCK && !MD.TreeCap.mLoaded && !aPlayer.level().isClientSide() && !aPlayer.isShiftKeyDown() && !aBlock.getClass().getName().startsWith("com.ferreusveritas.dynamictrees") && (aBlock instanceof HugeMushroomBlock || WD.wood(aBlock, aPlayer.level(), aX, aY, aZ) || OP.log.contains(ST.make(aBlock, 1, aMeta)) || WoodDictionary.WOODS.containsKey(aBlock, aMeta, T))) {
			LOCK = F;
			try {
				int tY = aY, tH = gregapi.util.WD.topY(aPlayer.level()) /* was getHeight()=COUNT */, tCount = 0, tIncrement = UT.Code.roundUp(WD.hardness(aBlock, aPlayer.level(), aX, aY, aZ) * getToolDamagePerBlockBreak());
				// Checking...
				while (++tY < tH) {
		...
		harvestStick(aDrops, aStack, aPlayer, aBlock, aAvailableDurability, aX, aY, aZ, aMeta, aFortune, aSilkTouch);
		return rAmount;
	}
	...
	public float getMiningSpeed(Block aBlock, byte aMeta, float aDefault, Player aPlayer, Level aWorld, int aX, int aY, int aZ) {
		...
		if (aBlock instanceof HugeMushroomBlock || WD.wood(aBlock, aPlayer.level(), aX, aY, aZ) || OP.log.contains(ST.make(aBlock, 1, aMeta)) || WoodDictionary.WOODS.containsKey(aBlock, aMeta, T)) {
			...
			if (!aPlayer.isShiftKeyDown() && !MD.TreeCap.mLoaded) for (int tY = aY+1, tH = gregapi.util.WD.topY(aPlayer.level()) /* was getHeight()=COUNT */; tY < tH; tY++) if (WD.block(aPlayer.level(), aX, tY, aZ) == aBlock) {tIncrement+=0.1F; rAmount+=tIncrement;} else break;
