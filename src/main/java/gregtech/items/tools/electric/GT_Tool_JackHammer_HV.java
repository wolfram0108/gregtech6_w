...
public class GT_Tool_JackHammer_HV extends GT_Tool_MiningDrill_LV {
	...
	public boolean isMinableBlock(Block aBlock, byte aMetaData) {
		return TOOL_pickaxe.equalsIgnoreCase(WD.harvestTool(aBlock, aMetaData)) || aBlock instanceof InfestedBlock || WD.getMaterial(aBlock) == Material.rock || WD.getMaterial(aBlock) == Material.glass || WD.getMaterial(aBlock) == Material.ice || WD.getMaterial(aBlock) == Material.packedIce;
	}
	...
	public int convertBlockDrops(List<ItemStack> aDrops, ItemStack aStack, Player aPlayer, Block aBlock, long aAvailableDurability, int aX, int aY, int aZ, byte aMetaData, int aFortune, boolean aSilkTouch, BlockDropsEvent aEvent) {
		int rConversions = 0;
		Recipe tRecipe;
		if (aBlock instanceof net.minecraft.world.level.block.EntityBlock || null == (tRecipe = RM.Hammer.findRecipe(null, null, true, Integer.MAX_VALUE, null, ZL_FS, ST.make(aBlock, 1, aMetaData)))) {
