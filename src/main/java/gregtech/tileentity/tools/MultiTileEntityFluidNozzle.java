				...
				if (FL.gas(aFluid, F) && aFluid.getAmount() > 0 && (mAcidProof || !FL.acid(aFluid))) {
					if (aStack == null) {
						...
						if (FL.XP.is(aFluid)) {
							if (MD.OB.mLoaded) {
								try {
									int tXP = Math.min(LiquidXpUtils.liquidToXpRatio(aFluid.getAmount()), UT.Code.roundUp(EnchantmentUtils.getExperienceForLevel(aPlayer.experienceLevel+1) - (EnchantmentUtils.getExperienceForLevel(aPlayer.experienceLevel)+(aPlayer.experienceProgress * aPlayer.getXpNeededForNextLevel()))));
