				...
				if (IBlockToolable.Util.onToolClick(TOOL_hammer, mEnergy * 10, mQuality, null, null, null, F, null, tD.mWorld, tD.mSideOfTileEntity, tD.mX, tD.mY, tD.mZ, 0.5F, 0.5F, 0.5F) > 0) {
					...
					if (!(tD.mTileEntity instanceof ITileEntityAnvil) || !((ITileEntityAnvil)tD.mTileEntity).isAnvil(tD.mSideOfTileEntity)) {
						...
						if (GT_Tool_HardHammer.INSTANCE.isMinableBlock(tBlock, tMetaData)) {
							mSendSound = 2;
							if (WD.harvestLevel(tBlock, tMetaData) <= mQuality) {
								float tHardness = WD.hardness(tBlock, tD.mWorld, tD.mX, tD.mY, tD.mZ);
								if (tHardness >= 0 && tHardness * 50 <= mEnergy) {
									List<ItemStack> tDrops = tD.mWorld instanceof net.minecraft.server.level.ServerLevel tSL ? net.minecraft.world.level.block.Block.getDrops(tSL.getBlockState(new net.minecraft.core.BlockPos(tD.mX, tD.mY, tD.mZ)), tSL, new net.minecraft.core.BlockPos(tD.mX, tD.mY, tD.mZ), WD.te(tD.mWorld, tD.mX, tD.mY, tD.mZ, T)) : new java.util.ArrayList<>(); // Was Block.getDrops(world,x,y,z,meta,fortune) in 1.7.10; neo uses static Block.getDrops(state,ServerLevel,pos,BE).
