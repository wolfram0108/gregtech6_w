...
public abstract class MultiTileEntityReactorCore extends TileEntityBase10FacingDouble implements ITileEntityServerTickPost, IFluidHandler, ITileEntityTapAccessible, ITileEntityFunnelAccessible, ITileEntityRunningActively, ITileEntitySwitchableOnOff, IMTE_GetCollisionBoundingBoxFromPool, IMTE_OnEntityCollidedWithBlock {
	...
	protected IFluidTank getFluidTankFillable2(byte aSide, FluidStack aFluidToFill) {
		return FL.Coolant_IC2.is(aFluidToFill)
			...
			|| FL.equal(MT.Sn.mLiquid, aFluidToFill)
			|| FL.equal(MT.Na.mLiquid, aFluidToFill)
			|| FL.equal(MT.HDO.mLiquid, aFluidToFill)
			|| FL.equal(MT.D2O.mLiquid, aFluidToFill)
			|| FL.equal(MT.T2O.mLiquid, aFluidToFill)
			|| FL.equal(MT.LiCl.mLiquid, aFluidToFill)
			|| FL.equal(MT.He.mGas, aFluidToFill)
			|| FL.equal(MT.CO2.mGas, aFluidToFill)
			? mTanks[0] : null;
	}
	...
	public int funnelFill(byte aSide, FluidStack aFluid, boolean aDoFill) {
		if (!( FL.Coolant_IC2.is(aFluid)
			...
			|| FL.equal(MT.Sn.mLiquid, aFluid)
			|| FL.equal(MT.Na.mLiquid, aFluid)
			|| FL.equal(MT.HDO.mLiquid, aFluid)
			|| FL.equal(MT.D2O.mLiquid, aFluid)
			|| FL.equal(MT.T2O.mLiquid, aFluid)
			|| FL.equal(MT.LiCl.mLiquid, aFluid)
			|| FL.equal(MT.He.mGas, aFluid)
			|| FL.equal(MT.CO2.mGas, aFluid)
			)) return 0;
