...
public class MultiTileEntityCrucible extends TileEntityBase10MultiBlockBase implements ITileEntityCrucible, ITileEntityEnergy, ITileEntityGibbl, ITileEntityWeight, ITileEntityTemperature, ITileEntityMold, ITileEntityServerTickPost, ITileEntityEnergyDataCapacitor, IMultiBlockEnergy, IMultiBlockInventory, IMultiBlockFluidHandler, IFluidHandler {
	...
	public boolean checkStructure2(BlockPos aCoordinates, Entity aPlayer, Container aInventory) {
		...
		if (getAir(getBlockPos().getX(), getBlockPos().getY()+1, getBlockPos().getZ())) WD.set(level, getBlockPos().getX(), getBlockPos().getY()+1, getBlockPos().getZ(), NB, 0, 3); else tSuccess = F;
		if (getAir(getBlockPos().getX(), getBlockPos().getY()+2, getBlockPos().getZ())) WD.set(level, getBlockPos().getX(), getBlockPos().getY()+2, getBlockPos().getZ(), NB, 0, 3); else tSuccess = F;
	...
	public boolean isInsideStructure(int aX, int aY, int aZ) {
		return aX >= getBlockPos().getX() - 1 && aY >= getBlockPos().getY() && aZ >= getBlockPos().getZ() - 1 && aX <= getBlockPos().getX() + 1 && aY <= getBlockPos().getY() + 2 && aZ <= getBlockPos().getZ() + 1;
	}
	...
	public void onServerTickPost(boolean aFirst) {
		...
		if (!slotHas(0)) slot(0, WD.suck(level, getBlockPos().getX()-0.5, getBlockPos().getY()+PX_P[2], getBlockPos().getZ()-0.5, 2, 3, 2));
		...
		if (mTemperature > getTemperatureMax(SIDE_INSIDE)) {
			UT.Sounds.send(SFX.MC_FIZZ, this, F);
			GarbageGT.trash(mContent);
			if (mTemperature >=  320) try {for (LivingEntity tLiving : (List<LivingEntity>)level.getEntitiesOfClass(LivingEntity.class, box(-GAS_RANGE, -1, -GAS_RANGE, GAS_RANGE+1, GAS_RANGE+1, GAS_RANGE+1))) UT.Entities.applyTemperatureDamage(tLiving, mTemperature, 4);} catch(Throwable e) {e.printStackTrace(ERR);}
