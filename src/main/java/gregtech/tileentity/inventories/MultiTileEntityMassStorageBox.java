...
public class MultiTileEntityMassStorageBox extends MultiTileEntityMassStorage {
	...
	public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
		...
		if (aSide == tF) {
			// Logical count via ST.count: a ZEROSIZE ghost stack shows as 0, not 1.
			return BlockTextureDefault.get(BI.decimalDigit(ST.count(slot(1)), 4-aRenderPass), ST.count(slot(1)) < mMaxStorage ? CA_WHITE : CA_RED_255, F, T, T, T);
		}
		return null;
	}
